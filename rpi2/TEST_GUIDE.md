...
1. **손을 초음파 센서 앞에 50cm 이내로 가져가기**
   - 예상 커널 로그: `[XX cm] 입차 차량 감지 — 게이트 OPEN, LED ON, 부저 ON`
...
1. **버튼 누르기** (GPIO 23)
   - 예상 로그: `[SWITCH] 출차 요청 — 게이트 OPEN, LED ON, 부저 ON, 10초 타이머 시작`
...
3. **손 치우기** (차량 빠져나감)
   - 예상 로그: `[XX cm] 차량 빠져나감 — 출차 완료, LED OFF, 부저 OFF, 게이트 닫음`
   - 실제: 게이트 닫힘, LED 꺼짐
...
- [ ] 센서 테스트: 손을 초음파 앞에 가져가기 → 로그 및 게이트 동작
- [ ] 버튼 테스트: GPIO 23 누르기 → 로그 및 출차 시나리오
- [ ] MQTT 만차 신호:
  ```bash
