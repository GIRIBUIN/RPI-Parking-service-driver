...
- **Dashboard 실시간 갱신**: MQTT over WebSocket으로 브라우저가 topic 직접 구독
...
역할:
- MQTT Broker 실행
- 브라우저 MQTT over WebSocket 연결 제공
- RPI1/RPI2 메시지 구독
- 슬롯 점유 상태 저장
- 게이트 상태 저장
- 이벤트 로그 저장
...
Broker 포트:
- `1883`: RPI1/RPI2 일반 MQTT 통신
- `9001`: Dashboard 브라우저 MQTT over WebSocket 통신
...
실시간 갱신 방식:
- 최초 화면 로딩 시 서버 API로 DB에 저장된 최신 상태를 한 번 조회한다.
- 이후에는 브라우저가 MQTT over WebSocket으로 topic을 직접 구독한다.
- 새 MQTT 메시지가 도착하면 Dashboard 화면을 즉시 갱신한다.
